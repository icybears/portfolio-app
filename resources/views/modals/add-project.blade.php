...
<form id="addProjectForm" method="post" enctype="multipart/form-data" action="{{ auth()->user()->getUsername() . '/projects' }}">
    ...
    <div class="form-group">
           ...
           <textarea name="description" class="form-control {{ $errors->has('description') ? ' is-invalid' : '' }}" id="description" placeholder="A short description" rows="4">{{ old('description') }}</textarea>
